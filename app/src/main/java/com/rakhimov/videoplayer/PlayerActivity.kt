            ...
            bindingMF.subtitleBtn.setOnClickListener {
                ...
                for (group in player.currentTracksInfo.trackGroupInfos){
                    if (group.trackType == C.TRACK_TYPE_TEXT){
                        val gropInfo = group.trackGroup
                        for (i in 0 until  gropInfo.length){
                            subtitles.add(gropInfo.getFormat(i).language.toString())
                            subtitlesList.add("${subtitlesList.size + 1}. " + Locale(gropInfo.getFormat(i).language.toString()).displayLanguage
                                    + " (${gropInfo.getFormat(i).label})")
                        }
                    }
                }
